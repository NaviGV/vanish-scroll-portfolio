...
router.post('/upload-image', auth, upload.single('projectImage'), async (req, res) => {
  try {
    if (!req.file) {
...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    project.tags = tags ? (typeof tags === 'string' ? tags.split(',').map(tag => tag.trim()) : tags) : project.tags;
    project.liveLink = liveLink !== undefined ? liveLink : project.liveLink;
    project.codeLink = codeLink !== undefined ? codeLink : project.codeLink;
